<script lang="ts">
	import { signUp, signIn, signInWithGoogle, signInWithTwitter, authError } from '$lib/stores/auth';
	...
	async function handleTwitterSignIn() {
		...
		try {
			...
			error = err.message || 'X sign in failed';
		} finally {
			loading = false;
		}
	}
...
<style>
	...
	.social-btn:hover:not(:disabled) {
		...
		border-color: rgba(200, 230, 180, 0.4);
		transform: translateY(-1px);
	}
